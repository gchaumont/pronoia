<template>
    <div v-intersect="intersect" v-if="searchInstance.results.hasMore">
        <button class="block mt-10 m-auto border px-3 py-2 rounded-lg border-gray-500 border-px flex items-center gap-2 hover:bg-slate-200 dark:hover:bg-slate-800 dark:text-slate-100" @click="loadMore">
...
const intersect = function (inter) {
    if (props.autoload && inter.isIntersecting) {
        loadMore()
    }
}
